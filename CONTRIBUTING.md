...
Thank you for your interest in contributing! This document covers how to get started, the conventions used in the codebase, and what to expect from the contribution process.
...
For a full walkthrough of the project structure, see [src/docs/quickstart-contributors.md](src/docs/quickstart-contributors.md).
...
This runs lint → format → type-check → test (with coverage) → build in sequence. All steps must pass.
...
- **TypeScript**: `strict: true`; all code must type-check cleanly. No `any` without justification.
- **CSS**: CSS Modules (`*.module.css`) co-located with each component. No preprocessors (except for `tokens.scss`).
...
There is no formal issue template, but please open an issue before starting large or architectural changes.

## License

By contributing, you agree that your contributions will be licensed under the [EUPL-1.2](LICENSE.txt).
