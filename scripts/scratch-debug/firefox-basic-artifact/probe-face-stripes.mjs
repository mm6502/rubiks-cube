...
console.log(
    `bright(>=${BRIGHT}) row spans: ${rowRuns.map(r => r[0] + '..' + r[1]).join(' ') || 'none'}`
);
console.log(
    `bright(>=${BRIGHT}) col spans: ${colRuns.map(r => r[0] + '..' + r[1]).join(' ') || 'none'}`
);

const pick = runs =>
    runs.filter(r => r[1] - r[0] >= MINW).sort((a, b) => b[1] - b[0] - (a[1] - a[0]))[0];
const rr = pick(rowRuns),
    cr = pick(colRuns);
if (!rr || !cr) {
    console.log('\nno face-sized bright region found');
    process.exit(0);
}
...
function scan(axis) {
    ...
    return {
        label:
            axis === 'rows'
                ? 'horizontal scan (bands run VERTICALLY)'
                : 'vertical scan (bands run HORIZONTALLY)',
...
for (const axis of ['rows', 'cols']) {
    ...
    console.log(
        `  dark runs: ${s.darkRunCount}  mean ${s.darkRunMean}px  p25 ${s.darkRunP25}  median ${s.darkRunMedian}  p75 ${s.darkRunP75}px`
    );
