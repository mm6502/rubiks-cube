...
for (const axis of ['rows', 'cols']) {
    ...
    const dir =
        axis === 'rows'
            ? 'horizontal scan (bands are VERTICAL)'
            : 'vertical scan (bands are HORIZONTAL)';
    console.log(`${dir}`);
    console.log(
        `  dark runs per line: mean ${s.meanDarkRunsPerLine}  range ${s.minRuns}..${s.maxRuns}`
    );
    console.log(
        `  dark run length   : mean ${s.meanRunLength}  median ${s.medianRunLength}  p10 ${s.p10}  p90 ${s.p90}`
    );
...
console.log(
    `  dominant GRADIENT direction ${gradAngle.toFixed(1)}deg from +x  ` +
