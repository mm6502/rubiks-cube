...
console.log(`${file}`);
console.log(`file ${meta.width}x${meta.height}  channels ${C}  region ${region.left},${region.top} ${W}x${H}`);
...
[...counts.entries()].sort((a, b) => b[1] - a[1]).slice(0, 8)
  .forEach(([k, n]) => console.log(`  ${k.padEnd(16)} ${String(n).padStart(8)}  ${(100 * n / total).toFixed(3)}%`));
console.log('');
...
for (const [name, [r, g, b]] of Object.entries(TOKENS)) {
  ...
  console.log(`  ${name.padEnd(26)} ${String(n).padStart(8)}  ${(100 * n / total).toFixed(3)}%`);
}
console.log('');
...
for (const axis of ['x', 'y']) {
  ...
  console.log(`profile along ${axis}: mean ${mean.toFixed(1)}  sd ${sd.toFixed(1)}  ` +
